 ...
 * Falls back to `other`, which is a real option rather than a dumping ground.
 */
export function detectVertical(industry?: string | null): string {
  ...
  // it a restaurant purely because restaurants are listed earlier. Specificity
  // is the honest tie-breaker — a longer keyword is a stronger signal.
...
/** True when an answer carries no information — used for progress, never to block. */
export function isAnswered(value: AnswerValue | undefined): boolean {
  ...
  // A price range may arrive with only some keys present (`{from: undefined}`
  // from a cleared field), so discriminate on ANY range key — testing for
  // "varies" alone let a half-empty range fall through to the hours branch.
