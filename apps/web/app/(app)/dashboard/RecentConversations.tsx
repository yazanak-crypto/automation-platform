...
function pill(status: string, pending: boolean) {
  if (pending || status === "waiting_approval")
    return { label: "Waiting", cls: "bg-wait-dim text-wait" };
  if (status === "closed") return { label: "Resolved", cls: "bg-ok-dim text-ok" };
  return { label: "Open", cls: "bg-hover text-ink-2" };
}

/** Concept's "Recent conversations" panel — real threads, real statuses. */
export default function RecentConversations() {
  const [rows, setRows] = useState<Row[] | null>(null);
  useEffect(() => {
    fetch("/api/conversations")
      .then((r) => (r.ok ? r.json() : []))
      .then((d: Row[]) => setRows(d.slice(0, 5)))
      .catch(() => setRows([]));
  }, []);
  ...
  return (
    <div className="lit rounded-[12px] border border-line bg-raised p-5">
      <div className="mb-2 flex items-center justify-between">
        ...
        <Link href="/conversations" className="text-[12px] text-ink-3 hover:text-ink-2">
          View all
        </Link>
      </div>
      {rows === null ? (
            ...
            return (
              <li key={r.id}>
                <Link
                  ...
                  className="flex items-center gap-3 border-b border-line py-2.5 last:border-0 hover:bg-hover"
                >
                  ...
                  <span className={`shrink-0 rounded-full px-2 py-0.5 text-[10.5px] font-medium ${p.cls}`}>
                    {p.label}
                  </span>
                </Link>
              </li>
