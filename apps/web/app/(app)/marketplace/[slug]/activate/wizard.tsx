...
export default function ActivateWizard({
  ...
  return (
    <main className="mx-auto max-w-xl p-8">
      <p className="text-sm text-ink-3">Activating</p>
      ...
      {step === 0 && (
        <section className="mt-8">
          ...
          {channels === null ? (
            <div className="mt-3 space-y-2" role="status" aria-label="Loading"><div className="skeleton h-14 w-full" /><div className="skeleton h-14 w-full" /></div>
          ) : channels.length === 0 ? (
            ...
            <ul className="mt-3 space-y-2">
              {channels.map((c) => (
                <li key={c.id}>
                  <label className="flex cursor-pointer items-center gap-3 rounded-xl border border-line p-4 has-[:checked]:border-line-strong">
                    <input
                      type="checkbox"
      ...
      {step === 1 && (
        <section className="mt-8 space-y-4">
          ...
          {configFields.map((f) => (
            <div key={f.key}>
              <label className="mb-1 block text-sm text-ink-2">{f.label}</label>
              {f.type === "textarea" ? (
                <textarea
                  className={inputCls}
                  rows={2}
              ...
              {f.help && <p className="mt-1 text-xs text-ink-3">{f.help}</p>}
            </div>
          ))}
          <div className="flex gap-3">
            <button onClick={() => setStep(0)} className="text-sm text-ink-3">← Back</button>
            <button
              onClick={() => setStep(2)}
      ...
      {step === 2 && (
        <section className="mt-8">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {sampleMessages.map((m) => (
              <button
                key={m}
                onClick={() => setSample(m)}
                className={`rounded-full px-3 py-1 text-xs ${sample === m ? "bg-white text-black" : "bg-hover text-ink-2"}`}
              >
                {m.slice(0, 40)}…
              </button>
            ))}
          ...
          {preview && (
            <div className="mt-4 rounded-xl border border-line bg-brass-dim p-4">
              {preview.reply ? (
                <>
          ...
          <div className="mt-6 flex gap-3">
            <button onClick={() => setStep(1)} className="text-sm text-ink-3">← Back</button>
            <button
              disabled={!preview}
              onClick={() => setStep(3)}
      ...
      {step === 3 && (
        <section className="mt-8">
          ...
          <div className="mt-4 space-y-2">
            <label className={`block cursor-pointer rounded-xl border p-4 ${mode === "supervised" ? "border-line-strong" : "border-line"}`}>
          ...
          <div className="mt-6 flex gap-3">
            <button onClick={() => setStep(2)} className="text-sm text-ink-3">← Back</button>
            <button
              disabled={busy}
              onClick={goLive}
              className="rounded-lg bg-ok/90 px-6 py-2.5 text-sm font-semibold text-black disabled:opacity-50"
            >
